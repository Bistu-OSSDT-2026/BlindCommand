...
TERRAIN_PROPS: Final[dict[TerrainType, TerrainProps]] = {
    ...
    # HQ 为可通行格，单位可进入（HQ 双占规则由 GameMap 管理）
...
MAP_DEFAULT_WIDTH:  Final[int] = 34    # 地图默认宽度（列数）
MAP_DEFAULT_HEIGHT: Final[int] = 25    # 地图默认高度（行数）
...
FOG_POSITION_ERROR_RADIUS:        Final[int] = 1   # 汇报坐标的随机误差（±1 格）
...
COMMAND_DELAY_MIN:       Final[float] = 0.5  # 最短传达延迟（秒）
COMMAND_DELAY_MAX:       Final[float] = 1.5  # 最长传达延迟（秒）

# ── 战斗系统 ────────────────────────────────────────────────────────────

COMBAT_MIN_DAMAGE:           Final[int]   = 1     # 单次攻击最小伤害
COMBAT_TYPE_ADVANTAGE_MULT:  Final[float] = 1.5   # 兵种克制伤害倍率
COMBAT_ROUND_INTERVAL:       Final[float] = 2.0   # 每轮攻防间隔（秒）
COMBAT_ENGAGE_DELAY:         Final[float] = 1.0   # 遭遇后延迟开战（秒）
COMBAT_ROUT_HP_RATIO:        Final[float] = 0.30  # 低于此比例敌军可能撤退
COMBAT_ROUT_CHANCE:          Final[float] = 0.40  # 撤退触发概率（每次 AI tick）

# ── RTT 移动 ──────────────────────────────────────────────────────────────

SECONDS_PER_TILE_BASE: Final[float] = 6.0  # 公式分母: 秒/格 = BASE / speed

# ── AI ────────────────────────────────────────────────────────────────────

AI_TICK_IDLE:   Final[float] = 3.0   # 空闲状态 AI tick 间隔（秒），中等等级默认
AI_TICK_COMBAT: Final[float] = 1.0   # 战斗状态 AI tick 间隔（秒）
AI_TICK_EASY:   Final[float] = 4.0   # 简单
AI_TICK_HARD:   Final[float] = 2.0   # 困难
REINFORCE_RANGE: Final[int] = 5      # 增援检测距离（格）

# ── 位置汇报（RTT） ──────────────────────────────────────────────────────

REPORT_INTERVAL_MIN:    Final[float] = 15.0  # 汇报间隔最小值（秒）
REPORT_INTERVAL_MAX:    Final[float] = 25.0  # 汇报间隔最大值（秒）
REPORT_CIRCLE_RADIUS:   Final[int]   = 2     # 汇报圈半径（格）
REPORT_CIRCLE_DURATION: Final[float] = 15.0  # 汇报圈显示时长（秒）
REPORT_CIRCLE_ALPHA:    Final[int]   = 80    # 汇报圈透明度（0-255）

# ── 指挥所占领 ──────────────────────────────────────────────────────────

CAPTURE_REQUIRED_TURNS: Final[int] = 2
CAPTURE_INTERRUPTIBLE:  Final[bool] = True
...
TERRAIN_IMAGE_FILES: Final[dict[TerrainType, str]] = {
    TerrainType.PLAIN:    "",          # 平原不贴符号
    TerrainType.FOREST:   "Tree1.png",
    TerrainType.MOUNTAIN: "Mountain1.png",
    TerrainType.RIVER:    "River.png",
    TerrainType.HQ_CELL:  "HQ_ally.png",
